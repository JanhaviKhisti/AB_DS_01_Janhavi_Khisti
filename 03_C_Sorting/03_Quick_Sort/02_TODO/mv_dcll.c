...
extern data_t mv_dcll_get_data_at( p_mv_dcll_t plist, size_t index)
{
    // Code
    #ifdef DEBUG
    ASSERTION(NULL != plist)
...
extern ret_t mv_dcll_add_data_at(p_mv_dcll_t plist, data_t new_data, size_t index)
{
    // Code
    #ifdef DEBUG
    ASSERTION(NULL != plist)
    ...
    if(index == 0)
        plist->p_head->p_next->key = new_data;
