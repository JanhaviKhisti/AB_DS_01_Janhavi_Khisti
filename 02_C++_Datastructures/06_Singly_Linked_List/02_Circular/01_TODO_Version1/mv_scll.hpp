...
public:
	...
	{ 
		...
		if(e_data == prunner->mp_next->m_data)
		{
			std::cerr << "ERROR : Noting Present before HEAD to remove.\n";
			return((T_SLL)NULL);
		}
		...
		while( true )
		{
			...
			if( mp_head == prunner->mp_next->mp_next )
			{
	...
	void ShowList()
	{
		...
		std::cout << "{START}->";
		for( prunner; mp_head != prunner; prunner = prunner->mp_next)
		{
	...
	/**
	 * Using Stack to reverse print the Singly Linked List
	 */
	void ReversePrint()
	{
		// Code
		...
		while( prunner != mp_head )
		{
			stack.Push(prunner->m_data);

			prunner = prunner->mp_next;
		}

		std::cout << "Reverse Singly List:" << std::endl;
		std::cout << "{START}->";
		while( stack.Top() )
		{
			std::cout << "{" << stack.Pop() << "}->";
