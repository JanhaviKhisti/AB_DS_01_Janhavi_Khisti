...
{
	if(object->object_type == MV_TYPE_EXPRESSION_NODE)
	{
		if(((p_mv_expression_node_t)object->object_value)->left)
		{
			expression_solver(((p_mv_expression_node_t)object->object_value)->left);
		} 

		if(((p_mv_expression_node_t)object->object_value)->right)
		{
			expression_solver(((p_mv_expression_node_t)object->object_value)->right);
		}

		printf("%s ", ((p_mv_expression_node_t)object->object_value)->operator->object_value); 
	}
	else
	{
		printf("%s ",object->object_value);	
	}
