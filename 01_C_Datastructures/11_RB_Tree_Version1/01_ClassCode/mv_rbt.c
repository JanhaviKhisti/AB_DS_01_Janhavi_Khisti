...
{
	// Code
	p_tree_node_t pnode = (p_tree_node_t)Xmalloc(SIZE_TREE_NODE);
	...
	while( RED == parent->color )
	{
		if( parent == g_parent->left )
		{
			...
			else 
			{
				...
				{
					// CASE 2: node and parent on opposite side of their parent respectively then rotate to parent side
					left_rotate(parent);
				}

				// CASE 3: node and parent on same side of their parent respectively then rotate to uncle side
				right_rotate(g_parent);
				node->color = BLACK;
				g_parent->color = RED;
			}
		}
		// parent is on right of g_parent
		else
		{
			uncle = g_parent->left;
...
static void delete_fixup(p_tree_node_t parent)
{
    // Code
    p_tree_node_t sibling = ( parent->right ? parent->right : parent->left );
...
static data_t generic_tree_remove(tree_t tree, data_t e_data, COMPAREDATAPROC pcomparedataproc)
{
	...
	p_tree_node_t prunner = locate_node(tree->root, e_data, pcomparedataproc);
	...
	// Loop ensures that the node to be deleted will be a node with NO children
	while( prunner->left || prunner->right )
	{
		 // CASE 1: prunner have LEFT CHILD
        if( NULL != prunner->left )
        {
        ...
        //  CASE 2: prunner have RIGHT CHILD
    ...
    if( NULL == prunner->parent )   // this means prunner is root
        ...
        prunner->parent->left = NULL;
    else
        prunner->parent->right = NULL;
    
    //  Call Delete fixup
    if( BLACK == prunner->color )
        delete_fixup(prunner->parent);
