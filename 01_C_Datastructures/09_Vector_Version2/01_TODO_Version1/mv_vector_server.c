...
extern  ret_t  mv_vector_merge(p_mv_vector_t pvector_1, p_mv_vector_t pvector_2)
{
	// Code
	ERROR_CHECKING( NULL != pvector_1,
					NULL == pvector_1,
					"ERROR : First Vector NOt Found\n");

	ERROR_CHECKING( NULL != pvector_2,
					NULL == pvector_2,
					"ERROR : Second Vector NOt Found\n");

	size_t total_elements = pvector_1->nr_of_elements + pvector_2->nr_of_elements;

	for( size_t le = 0; le < pvector_2->nr_of_elements; ++le )
	{
		if( pvector_1->capacity <= total_elements )
		{
			pvector_1->capacity = pvector_1->capacity * 2;
			pvector_1->array = (p_data_t) Xrealloc(pvector_1->array, pvector_1->capacity * SIZE_DATA);
		} 

		pvector_1->array[ pvector_1->nr_of_elements ] = pvector_2->array[ le ];

		pvector_1->nr_of_elements++;
	}
